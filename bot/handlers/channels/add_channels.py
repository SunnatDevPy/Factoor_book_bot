...
from bot.detail_text import channel_detail
from bot.filters.is_admin import IsAdmin
from bot.buttuns.inline import show_channels, confirm_channels
from config import BOT
from state.states import AddChannelState, ForwardState

channels_db = RedisDict('channels_db')
bot_db = RedisDict('bot_db')

ADMINS = int(BOT.ADMIN),
...
@channels_router.message(Command('add'), IsAdmin())
async def add_channel(message: Message, state: FSMContext):
    await state.set_state(AddChannelState.chat_id)
    await message.answer('Chat id ni kiriting')


@channels_router.message(AddChannelState.chat_id, IsAdmin())
...
@channels_router.callback_query(AddChannelState.confirm, IsAdmin())
...
@channels_router.message(Command('channels'), IsAdmin())
...
@channels_router.callback_query(F.data.startswith('clear_channel'), IsAdmin())
